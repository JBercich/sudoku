...
class BacktrackingSolver(Solver):
    ...
    def _solve(cls, grid: Grid, **params) -> Grid:
        """Generic backtracking method for solving a sudoku algorithm."""

        cls._solve_recursion(grid, 0, 0)

        return grid

    @classmethod
    def _solve_recursion(cls, grid: Grid, row: int, col: int) -> bool:
        ...
        if row >= Grid.ROWS or col >= Grid.COLS:
            return True
        next_col: int = col + 1 if col < Cell._MAX - 1 else Cell._MIN
        next_row: int = row + 1 if next_col == Cell._MIN else row

        # Skip the cell if it is a static fixed cell
        if grid[row, col].static:
            return cls._solve_recursion(grid, next_row, next_col)

        for next_value in range(Cell._MAX + 1):
            if cls.validate_update(grid, row, col, next_value):
                grid[row, col] = next_value
                is_solved: bool = cls._solve_recursion(grid, next_row, next_col)
                if is_solved:
                    return True
                else:
                    continue
    ...
    def validate_col_update(cls, grid: Grid, col: int, value: int) -> bool:
        ...
        return value not in [cell.value for cell in grid.get_col(col)]
    ...
    def validate_grid_update(cls, grid: Grid, row: int, col: int, value: int) -> bool:
        ...
        return value not in [cell.value for cell in grid.get_grid(row, col)]
